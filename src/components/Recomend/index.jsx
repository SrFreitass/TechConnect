...
import { getDocs, collection, where, query, orderBy, limit, startAfter } from "firebase/firestore";
...
export function Recomend({ category, title }) {
    ...
    useEffect(() => {
        const fetchRecommends = async () => {
            const q = query(collection(db, "articles"), where("title", "!=", title), where("category", "==", category), limit(4))
                        ...
                        <NewsStyled key={index}>
                            ...
                            <div>
                                <Link to={`../home/news/${article.id}`}>
                                    <h3 dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(article.title) }} />
                                </Link>
